 ...
 */
 ...
 ////	ICT Department
 ////	Updated from Petrit Rama and Edlira Zherka for ICT Portal System
...
class jqgrid
{
	...
	function set_options($options)
	{
		...
		if ($this->options["form"]["position"] == "center")
		{
			...
			$this->options["add_new_options"]["beforeShowForm"] = 'function (formid) 
									{
										'.$show_form_nav.'
			...
			$this->options["handover_options"]["beforeShowForm"] = 'function (formid) 
									{
										'.$show_form_nav.'

										// align dialog to center
										var gid = formid.attr("id").replace("FrmGrid_","");
										var dlgDiv = $("#handmod" + gid);
										var parentDiv = dlgDiv.parent(); // div#gbox_list
										var dlgWidth = dlgDiv.width();
										var parentWidth = parentDiv.width();
										var dlgHeight = dlgDiv.height();
	...
	function render($grid_id)
	{
			...
			switch($op)
			{
				case "autocomplete":
					...
					foreach( $this->options["colModel"] as $c )
					{
						if ($c["index"] == $field)
						{
							$sql = $c["formatoptions"]["sql"]. " WHERE {$c['formatoptions']['search_on']} like 'tuki'";
				...
				case "add":
					...
					$update_logs = implode(", ",$update_logs);
					$update_logs = str_replace("'","",$update_logs);
					...
					if($this->table=='asset_table')
						$dir = 'inventory';
					else if($this->table=='maintenance_table')
						$dir = 'maintenance';
					else if($this->table=='payments_table')
						$dir = 'payments';
					else if($this->table=='projects_table')
						$dir = 'projects';
					else if($this->table=='tasks_table')
						$dir = 'tasks';
						
					$sql_logs = "INSERT INTO logs_table VALUES (NULL, CURRENT_TIMESTAMP, '$ip', '$user', '$dir', 'add','$update_logs','----', false)";
				...
				case "edit":
					...
					$sql_edit = "SELECT * FROM $this->table WHERE id = $id";
					$result = $this->execute_query($sql_edit);
					...
					foreach($data as $k=>$v)
					{
						...
						$vv = $v;
						$v = ($v == "NULL") ? $v : "'$v'";
						$row[$k] = ($row[$k] == "NULL") ? $row[$k] : "'$row[$k]'";
						
						if($k != 'registered' && $k != 'log_in' && $k != 'log_out')
						{
							$update_str[] = "$k=$v";
						}
					...
					$sql = "UPDATE {$this->table} $update_str WHERE id = $id";
					...
					$sql_logs = "INSERT INTO logs_table VALUES (NULL, CURRENT_TIMESTAMP, '$ip', '$user', '$dir', 'edit','$update_logs','$edited', false)";
					$this->execute_query($sql_logs);
				break;
				...
				case "del":
					...
					$sql = "DELETE FROM {$this->table} WHERE id=".$_POST['id']." s";
		...
		if ($this->actions["rowactions"] !== false)
		{
			...
			if ($f)
				$action_column = array("name"=>"act", "align"=>"center", "index"=>"act", "width"=>"100", "sortable"=>false, "search"=>false, "viewable"=>false);
			else
	...
	?>
        ...
        <?php if ($this->actions["add_new"] !== false) { ?>
		// add *new* record 
		jQuery("#<?php echo $grid_id?>").jqGrid('navButtonAdd',"#<?php echo $grid_id."_pager"?>",{caption:"",title:"Add New Record", buttonicon :'ui-icon-plus',
			onClickButton:function()
			{
				var myGrid = $('#<?php echo $grid_id?>');
				selRowId = myGrid.jqGrid ('getGridParam', 'selrow');
				
				unit = myGrid.jqGrid ('getCell', selRowId, 'unit3');
				status = myGrid.jqGrid ('getCell', selRowId, 'status3');
				type = myGrid.jqGrid ('getCell', selRowId, 'type3');
				cname = myGrid.jqGrid ('getCell', selRowId, 'cname3');
				responsible = myGrid.jqGrid ('getCell', selRowId, 'responsible3');
				hardware = myGrid.jqGrid ('getCell', selRowId, 'hardware3');
				os = myGrid.jqGrid ('getCell', selRowId, 'os3');
				office = myGrid.jqGrid ('getCell', selRowId, 'office3');
				antivirus = myGrid.jqGrid ('getCell', selRowId, 'antivirus3');
				applications = myGrid.jqGrid ('getCell', selRowId, 'applications3');
				leid = myGrid.jqGrid ('getCell', selRowId, 'leid3');
				modules = myGrid.jqGrid ('getCell', selRowId, 'modules3');
				sas = myGrid.jqGrid ('getCell', selRowId, 'sas3');
				cs = myGrid.jqGrid ('getCell', selRowId, 'cs3');
				other = myGrid.jqGrid ('getCell', selRowId, 'other3');
				notes = myGrid.jqGrid ('getCell', selRowId, 'notes3');
				...
				//antivirus
				var antivirus2 = [];
				$("#antivirus21").change(checkAntivirus);
				$("#antivirus22").change(checkAntivirus);
				$("#antivirus23").change(checkAntivirus);
				$("#antivirus24").change(checkAntivirus);
				$("#antivirus25").change(checkAntivirus);
				
				function checkAntivirus()
				{
					antivirus2 = [];
					if($('#antivirus21').prop('checked')) {antivirus2.push($('#antivirus21').val());}
					if($('#antivirus22').prop('checked')) {antivirus2.push($('#antivirus22').val());}
					if($('#antivirus23').prop('checked')) {antivirus2.push($('#antivirus23').val());}
					if($('#antivirus24').prop('checked')) {antivirus2.push($('#antivirus24').val());}
					if($('#antivirus25').prop('checked')) {antivirus2.push($('#antivirus25').val());}
					$('#antivirus2').val(antivirus2);
				}
				//applications
				var applications2 = [];
				$("#applications21").change(checkApp);
				$("#applications22").change(checkApp);
				$("#applications23").change(checkApp);
				$("#applications24").change(checkApp);
				$("#applications25").change(checkApp);
				$("#applications26").change(checkApp);
				$("#applications27").change(checkApp);
				$("#applications28").change(checkApp);
				
				function checkApp()
				{
					applications2 = [];
					if($('#applications21').prop('checked')) {applications2.push($('#applications21').val());}
					if($('#applications22').prop('checked')) {applications2.push($('#applications22').val());}
					if($('#applications23').prop('checked')) {applications2.push($('#applications23').val());}
					if($('#applications24').prop('checked')) {applications2.push($('#applications24').val());}
					if($('#applications25').prop('checked')) {applications2.push($('#applications25').val());}
					if($('#applications26').prop('checked')) {applications2.push($('#applications26').val());}
					if($('#applications27').prop('checked')) {applications2.push($('#applications27').val());}
					if($('#applications28').prop('checked')) {applications2.push($('#applications28').val());}
					$('#applications2').val(applications2);
				}	
				//modules
				var modules2 = [];
				$("#modules21").change(checkModules);
				$("#modules22").change(checkModules);
				$("#modules23").change(checkModules);
				$("#modules24").change(checkModules);
				$("#modules25").change(checkModules);
				$("#modules26").change(checkModules);
				$("#modules27").change(checkModules);
				
				function checkModules()
				{
					modules2 = [];
					if($('#modules21').prop('checked')) {modules2.push($('#modules21').val());}
					if($('#modules22').prop('checked')) {modules2.push($('#modules22').val());}
					if($('#modules23').prop('checked')) {modules2.push($('#modules23').val());}
					if($('#modules24').prop('checked')) {modules2.push($('#modules24').val());}
					if($('#modules25').prop('checked')) {modules2.push($('#modules25').val());}
					if($('#modules26').prop('checked')) {modules2.push($('#modules26').val());}
					if($('#modules27').prop('checked')) {modules2.push($('#modules27').val());}
					$('#modules2').val(modules2);
				}	
				//sas
				var sas2 = [];
				$("#sas21").change(checkSAS);
				$("#sas22").change(checkSAS);
				$("#sas23").change(checkSAS);
				$("#sas24").change(checkSAS);
				
				function checkSAS()
				{
					sas2 = [];
					if($('#sas21').prop('checked')) {sas2.push($('#sas21').val());}
					if($('#sas22').prop('checked')) {sas2.push($('#sas22').val());}
					if($('#sas23').prop('checked')) {sas2.push($('#sas23').val());}
					if($('#sas24').prop('checked')) {sas2.push($('#sas24').val());}
					$('#sas2').val(sas2);
				}
				...
				{
					cs2 = [];
					if($('#cs21').prop('checked')) {cs2.push($('#cs21').val());}
					if($('#cs22').prop('checked')) {cs2.push($('#cs22').val());}
					if($('#cs23').prop('checked')) {cs2.push($('#cs23').val());}
					$('#cs2').val(cs2);
				}
				...
				document.getElementById("responsible2").value = responsible;
				document.getElementById("hardware2").value = hardware;
				document.getElementById("os2").value = os;
				document.getElementById("office2").value = office;
				document.getElementById("antivirus2").value = antivirus;
				document.getElementById("applications2").value = applications;
				document.getElementById("leid2").value = leid;
				document.getElementById("modules2").value = modules;
				document.getElementById("sas2").value = sas;
				document.getElementById("cs2").value = cs;
				document.getElementById("other2").value = other;
				document.getElementById("notes2").value = notes;
		...
		<?php } ?>
		<?php if ($this->actions["add_maintenance"] !== false) { ?>
		...
		jQuery("#<?php echo $grid_id?>").jqGrid('navButtonAdd',"#<?php echo $grid_id."_pager"?>",{caption:"",title:"Add New Record", buttonicon :'ui-icon-plus',
			onClickButton:function()
			{
				...
				planned_date = myGrid.jqGrid ('getCell', selRowId, 'planned_date');
				completed_date = myGrid.jqGrid ('getCell', selRowId, 'completed_date');
				duration = myGrid.jqGrid ('getCell', selRowId, 'duration');
				contractor = myGrid.jqGrid ('getCell', selRowId, 'contractor');
				tel_email = myGrid.jqGrid ('getCell', selRowId, 'tel_email');
				scope = myGrid.jqGrid ('getCell', selRowId, 'scope');
				contract_startdate = myGrid.jqGrid ('getCell', selRowId, 'contract_startdate');
				contract_enddate = myGrid.jqGrid ('getCell', selRowId, 'contract_enddate');
				notes = myGrid.jqGrid ('getCell', selRowId, 'notes');
				doc_link = myGrid.jqGrid ('getCell', selRowId, 'doc_link');
				...
				document.getElementById("planned_date1").value = planned_date;
				document.getElementById("completed_date1").value = completed_date;
				document.getElementById("duration1").value = duration;
				document.getElementById("contractor1").value = contractor;
				document.getElementById("tel_email1").value = tel_email;
				document.getElementById("scope1").value = scope;
				document.getElementById("contract_startdate1").value = contract_startdate;
				document.getElementById("contract_enddate1").value = contract_enddate;
				document.getElementById("notes1").value = notes;
				document.getElementById("doc_link1").value = doc_link;
		...
		<?php if ($this->actions["spare"] !== false) { ?>
		// spare parts
		jQuery("#<?php echo $grid_id?>").jqGrid('navButtonAdd',"#<?php echo $grid_id."_pager"?>",{caption:"Spare Parts",title:"Set the number of spare parts", buttonicon :'ui-icon-flag',
			onClickButton:function()
			{
				var myGrid = $('#<?php echo $grid_id?>');
				selRowId = myGrid.jqGrid ('getGridParam', 'selrow');
				
				sid = myGrid.jqGrid('getCell', selRowId, 'id');
				min = myGrid.jqGrid ('getCell', selRowId, 'min_nr_spare');
				
				if(typeof sid!=="undefined")
				{
					spare = document.getElementById("sspare"+sid).innerHTML;
					if(myGrid.jqGrid ('getCell', selRowId, 'asset_condition') == "Spare")
					{
						document.getElementById("sid").value = sid;
						document.getElementById("spare").value = spare;
						document.getElementById("spare1").value = spare;
						...
						$( "#dialog_min" ).dialog({
							  ...
							  modal:true,
								...
								{
								  text: "Submit",
								  click: function() {$( "#minSpareForm" ).submit();}
								},
								{
								  text: "Cancel",
								  click: function() {$( this ).dialog( "close" );}
		...
		<?php if ($this->actions["handover"] !== false) { ?>
		// handover
		jQuery("#<?php echo $grid_id?>").jqGrid('navButtonAdd',"#<?php echo $grid_id."_pager"?>",{caption:"Handover",title:"Handover Form", buttonicon :'ui-icon-note',
			onClickButton:function()
			{
				var myGrid = $('#<?php echo $grid_id?>');
				selRowId = myGrid.jqGrid ('getGridParam', 'selrow');
				...
				unit = myGrid.jqGrid ('getCell', selRowId, 'unit');
				name = myGrid.jqGrid ('getCell', selRowId, 'name');
				condition = myGrid.jqGrid ('getCell', selRowId, 'asset_condition');
				vendor = myGrid.jqGrid ('getCell', selRowId, 'vendor');
				description = myGrid.jqGrid ('getCell', selRowId, 'description');
				ass_number = myGrid.jqGrid ('getCell', selRowId, 'asset_nr');
				serial = myGrid.jqGrid ('getCell', selRowId, 'serial_nr');
				responsible = myGrid.jqGrid ('getCell', selRowId, 'responsible');
				building = myGrid.jqGrid ('getCell', selRowId, 'building');
				floor = myGrid.jqGrid ('getCell', selRowId, 'floor');
				number = myGrid.jqGrid ('getCell', selRowId, 'number');
				notes = myGrid.jqGrid ('getCell', selRowId, 'notes');
				if(typeof responsible!=="undefined")
				{
					...
					document.getElementById("ass_number").value = ass_number;
					...
					document.getElementById("condition1").value = condition;
					document.getElementById("responsible1").value = responsible;
					document.getElementById("building1").value = building;
					...
					$( "#dialog" ).dialog({
						  ...
						  modal:true,
							...
							{
							  text: "Submit",
							  click: function() {$( "#handoverForm" ).submit();}
							},
							{
							  text: "Cancel",
							  click: function() {$( this ).dialog( "close" );}
		...
		function link_date_picker(el,fmt,toolbar)
		{
			...
			setTimeout(function(){
				...
				{ 
					...
					{ 
						if (toolbar == 0) jQuery(el).after('<button>Calendar</button>').next().button({icons:{primary: 'ui-icon-calendar'}, text:false}).css({'font-size':'69%'}).click(function(e){jQuery(el).datepicker('show');return true;});
		...
		function link_datetime_picker(el,fmt)
		{
			setTimeout(function(){
				...
				{ 
					if(jQuery.ui.datepicker) 
					{
						jQuery(el).after('<button id="timepicker">Calendar</button>').next().button({icons:{primary: 'ui-icon-calendar'}, text:false}).css({'font-size':'69%'}).click(function(e){jQuery(el).datetimepicker('show');return false;});
