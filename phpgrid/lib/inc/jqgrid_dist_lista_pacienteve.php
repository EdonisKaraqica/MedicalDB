 ...
 */
 ...
 ////	ICT Department
 ////	Updated from Petrit Rama for Inventory Database Project
 ////   true
...
class jqgrid
{
	...
	function set_columns($cols = null)
	{
		...
		for($i=0;$i<count($cols);$i++)
		{
			...
			// qitu nuk pe merr titullin e kolonave!!
	...
	function render($grid_id)
	{
			...
			switch($op)
			{
				...
				case "add":
					...
					// remove any table alias from query - obseleted
						...
						if($k=='time')
							$v="DATEDIFF('$due_date','$init_date')";
						else if($k=='proficiency')
						{
							if($comp_date=='NULL')
								$v="''";
							else
								$v="DATEDIFF('$due_date','$comp_date')";
						}
						else
							$v = ($v == "NULL") ? $v : "'$v'";

						if($k!='tid' && $k!='time' && $k!='proficiency')
							$update_logs[] = "$k=$vv";
						$values_str[] = "$v";
					}
				...
				case "edit":
					...
					$sql_edit = "SELECT * FROM asset_table WHERE $pk_field = $id";
				...
				case "del":
					...
					$sql = "DELETE FROM {$this->table} WHERE $pk_field IN ($id) s";
		...
		if (isset($_GET['jqgrid_page']))
		{
			...
			$sidx = $_GET['pid']; // get index row - i.e. user click to sort
			...
			if (isset($_GET["export"]))
			{
				...
				//session_start();
				$exprange = $_SESSION["exprange"];
				//$export_where = "";
				//echo $exprange;

				//echo $this->options["export"]["range"];
				//echo "Inter";

				if ($exprange == "filtered"){
					$export_where = $_SESSION["jqgrid_filter"];
					//echo '<script language="javascript">';
					//echo 'alert("Ka hi ne if")';
					//echo '</script>';
				}
				...
				if ($this->options["export"]["format"] == "pdf")
				{
					...
					else
					{
						...
						$html .= '<th><strong>Emri</strong></th>
											<th><strong>Mbiemri</strong></th>
											<th><strong>Gjinia</strong></th>
											<th><strong>Ditelindja</strong></th>
											<th><strong>Vendlindja</strong></th>
											<th><strong>Dept</strong></th>
											<th><strong>Njesia</strong></th>
											<th><strong>Mbikqyresi</strong></th>
											<th><strong>Telefoni</strong></th>';
						...
						$html .= '</tr>'; // me kta u perfundu kolona e titujve

						$j = 0;

						$xkolonat = count(current($cols_not_to_export));
						//$html .= '<tr><td>' . $kolonat;
						//$html .= $xkolonat . '</td></tr>';
	...
	?>
		...
		jQuery("#<?php echo $grid_id?>").jqGrid('navButtonAdd',"#<?php echo $grid_id."_pager"?>",{caption:"Handover",title:"Handover Form", buttonicon :'ui-icon-note',
			onClickButton:function()
			{
				...
				condition = myGrid.jqGrid ('getCell', selRowId, 'condition');
				vendor = myGrid.jqGrid ('getCell', selRowId, 'vendor');
				description = myGrid.jqGrid ('getCell', selRowId, 'description');
				ass_number = myGrid.jqGrid ('getCell', selRowId, 'ass_number');
				serial = myGrid.jqGrid ('getCell', selRowId, 'serial');
