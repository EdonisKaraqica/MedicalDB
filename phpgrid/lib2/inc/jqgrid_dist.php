				...
				foreach($rules as $key=>$val)
				{
					...
					if(isset($v) && isset($op))
					{
						...
						switch ($op) {
							...
							case 'cn' :
							case 'bw' :
								//$field = ($field=="FLIGHT1")?"CONCAT(reel_journees.CIE,reel_journees.LIG)":$field;
								//$field = ($field=="FLIGHT1_ROT")?"CONCAT(reel_journees.CIE_ROT,reel_journees.LIG_ROT)":$field;
								//$field = ($field=="DATEVOL")?"reel_journees.DATEVOL":$field;
								//$field = ($field=="DATEVOL_ROT")?"reel_journees.DATEVOL_ROT":$field;
								//$field = ($field=="IMMAT")?"reel_journees.IMMAT":$field;
								//$field = ($field=="CALLSIGN")?"reel_journees.CALLSIGN":$field;
								/*if($field=="IMMAT" && $this->table!="lst_immat")
									$field = "reel_journees.DATEVOL_ROT";*/
	...
	function render($grid_id)
	{
			...
			switch($op)
			{
				...
				case "add":
					...
					foreach($data as $k=>$v)
					{
						...
						$k = addslashes($k);
						$v = addslashes($v);
						$k = ($k == "flight_date1") ? "flight_date" : $k;
						$fields_str[] = "$k";
		...
		if (isset($_GET['jqgrid_page']))
		{
			...
			if (($p = stripos($this->select_command,"GROUP BY")) !== false)
			{
				...
				$sql_count = "SELECT count(*) as c FROM ($start_q $wh $end_q)";
			}
			else
			{
