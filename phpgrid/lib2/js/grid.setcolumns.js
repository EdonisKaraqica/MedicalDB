...
$.jgrid.extend({
	setColumns : function(p) 
	{
		p = $.extend({
			...
			ShrinkToFit : true,
			jqModal : false,
			saveicon: [false,"left","ui-icon-disk"],
		...
		return this.each(function()
		{
			...
			else 
			{
				...
				for(i=13; i<this.p.colNames.length; i++)
				{
					if(!$t.p.colModel[i].hidedlg)
					{
						formdata += "<tr><td style='white-space: pre;'><input type='checkbox' style='margin-right:5px;margin-left:10px;' id='col_" + this.p.colModel[i].name + "' class='cbox' value='T' " + ((this.p.colModel[i].hidden==false)?"checked":"") + "/>" +  "<label for='col_" + this.p.colModel[i].name + "'>" + this.p.colNames[i].replace("<br>"," ") + "</label></td></tr>";
					}
				}
				...
				bC = "<a href='javascript:void(0)' id='eData' class='fm-button ui-state-default ui-corner-all'>Close</a>";
				...
				if(!p.updateAfterCheck) 
				{
					...
					{
						for(i=8;i<$t.p.colModel.length;i++)
						{
							if(!$t.p.colModel[i].hidedlg)
							{
