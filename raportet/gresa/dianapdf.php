...
class createPDF 
{
    ...
    function __construct($post) 
    {
        ...
        //variablat statike 
        $this->titulli = "LIMAK KOSOVO IINTERNATIONAL AIRPORT J.S.C.";
    ...
    function run() 
    {
		...
		$pdf->Cell(2);
		$pdf->MultiCell(116,5,$this->anamneza,0,"L",false);
    
		$pdf->Cell(500,3,"",0,1);
		$pdf->WriteHTML($this->ekzaminimi_fizikal);
		$pdf->Cell(9.1);
		$pdf->MultiCell(116,5,'',0,"L",false);
		...
		if (!empty($_POST["diagnozaRec"])||!empty($_POST["rp"])) 
		{
			...
			$pdf->Cell(2);
			$pdf->MultiCell(157,5,$this->receta,0,'L',false);
			$pdf->Cell(3,5,"",0,1);
  		...
  		if (!empty($_POST["udhPer"])||!empty($_POST["diagnoza"])||!empty($_POST["gjPrez"])||!empty($_POST["terapiaAp"])) 
  		{
        	...
        	$pdf->WriteHTML($this->diagnoza);
			$pdf->Cell(24.5);
			$pdf->MultiCell(135,5,$this->diagnozaKonsult,0,'L',false);
			$pdf->Cell(3,5,"",0,1);
		...
		if (!empty($_POST["udhPerLab"])||!empty($_POST["diagnozaLab"])||!empty($_POST["gjPrezLab"])||!empty($_POST["terLab"])) 
		{
			...
			$pdf->Cell(23.3);
			$pdf->MultiCell(136,5,$this->diagnozaLab,0,'L',false);
			$pdf->Cell(3,5,"",0,1);
	...
	if (!empty($_POST["udhezohetPerRent"])||!empty($_POST["diagnozaRent"])||!empty($_POST["gjPrezenteRent"])||!empty($_POST["terapiaApRent"])) 
	{
		...
		$pdf->WriteHTML($this->diagnozau);
		$pdf->Cell(24);
		$pdf->MultiCell(135,5,$this->diagnozaRent,0,'L',false);
		$pdf->Cell(3,5,"",0,1);
	...
	if (!empty($_POST["origjina"])||!empty($_POST["destinacioni"])||!empty($_POST["shenime"])) 
	{
		...
		$pdf->Cell(3.2);
		$pdf->Cell(30,5,$this->origjina,0,0);
		//$pdf->Cell(2,13,"",0,0);
     
		$pdf->WriteHTML($this->destinimip);
		$pdf->Cell(3.5);
		$pdf->Cell(62.5,5,$this->destinacioni,0,1);
		$pdf->Ln(5);
...
class PDF extends FPDF
{
    ...
    function Footer()
    {
        ...
        $this->Cell(0,4,$this->emriDok.' '.$this->mbiemriDok.', '. $this->data.' '.date("h:i:sa"),0,1,'C');
    ...
    function Header()
    {
		...
		$this->SetFont('Arial','',10);
        $this->Cell(137,10,$this->articleurl,0,0,'R');

        $this->Ln(4);
        $this->Cell(134,10,$this->url1,0,0,'R');
        $this->Ln(5);
